...
function append_features_main(model_data::Model_data,output_options::Output_options,
        name,colorR,colorG,colorB,border,border_remove_objs,min_area,parent)
    push!(model_data.features,Feature(String(name),Int64.([colorR,colorG,colorB]),
        Bool(border),Bool(border_remove_objs),Int64(min_area),
        String(parent),output_options))
    return nothing
end
append_features(name,colorR,colorG,colorB,border,border_remove_objs,min_area,parent) =
    append_features_main(model_data,output_options,name,colorR,colorG,
    colorB,border,border_remove_objs,min_area,parent)

# Updates model feature with new data
function update_features_main(model_data,index,name,colorR,colorG,colorB,
        border,border_remove_objs,min_area,parent)
